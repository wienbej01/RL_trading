...
class TestPPOLSTMPolicy:
    ...
    def setup_method(self):
        ...
        self.observation_space = gym.spaces.Box(low=-1, high=1, shape=(self.obs_dim,))
...
class TestRLTrainer:
    ...
    def test_trainer_initialization(self):
        """Test trainer initialization."""
        settings = Settings({'train': {'learning_rate': 0.0003}})
    ...
    def test_model_persistence_flow(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            def lr_schedule(progress):
                return 0.001
            policy = PPOLSTMPolicy(observation_space=observation_space, action_space=action_space, lr_schedule=lr_schedule)
            policy.save(model_path)
            
            # Load model in new instance
            new_policy = PPOLSTMPolicy.load(model_path)
            
            # Test that both models produce similar outputs
            test_obs = torch.randn(1, 5, 10)
            
            with torch.no_grad():
                output1 = policy.forward(test_obs)
                output2 = new_policy.forward(test_obs)
